...
# Admin Credentials (Aap yahan apna pasandeeda username/password rakh sakte hain)
ADMIN_USERNAME = os.getenv("ADMIN_USERNAME", "admin")
ADMIN_PASSWORD = os.getenv("ADMIN_PASSWORD", "nexus2026")
...
def send_email_notification(lead_data: dict, status: str):
    ...
    if is_qualified:
        ...
        <br><p>Best regards,<br><strong>Nexus Flow Engineering Team</strong></p>
        """
    else:
        ...
        <br><p>Best regards,<br><strong>Nexus Flow Team</strong></p>
        """
    try:
        resend.Emails.send({
...
def admin_dashboard(username: str = Depends(verify_admin)):
    ...
    <head>
      ...
      <style>
        ...
        .btn { background: #0284c7; color: #fff; border: none; padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; }
        ...
        th { background-color: #1e293b; color: #94a3b8; font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    <body>
      <div class="header">
        <h1>Nexus Flow — Secure Lead Dashboard</h1>
        <div class="actions">
          <button class="btn" onclick="fetchLeads()">Refresh Leads</button>
        </div>
      </div>
      <table>
        <thead>
        ...
        <tbody id="leads-table-body">
          <tr><td colspan="8" style="text-align:center;">Loading secure leads...</td></tr>
        </tbody>
      </table>
      <script>
          ...
          try {
            ...
            if (data.success && data.leads) {
              if (data.leads.length === 0) {
                tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;">No leads captured yet.</td></tr>';
                return;
              }
              tbody.innerHTML = data.leads.map(lead => {
                ...
                return `
                  <tr>
                    ...
                    <td>
                      <button class="btn btn-danger" onclick="deleteLead('${lead.id}')" style="padding: 6px 12px; font-size: 12px;">Delete</button>
                    </td>
                  </tr>
                `;
              }).join('');
            } else {
              tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#ef4444;">Error fetching leads.</td></tr>';
            }
          } catch (err) {
            tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#ef4444;">Failed to connect to backend API.</td></tr>';
          }
        }
